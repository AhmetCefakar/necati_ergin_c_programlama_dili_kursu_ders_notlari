...
int main(void)
{
    char str[10][20] = { "adsyad1", "adsyad2" }; // Buradaki 'str' dizisi elemanları 'char[20]' türünden olan diziler ve 'str' dizisinin elemanları programın başındna sonuna kadar kalan ve 'const' olan diziler değildir, burada 'otomatic storage duration' ömürlü oldukları için kapsamları dışında bellekten silinecektir.
}
```



👇 Aşağıdaki programda mülakatlarda iki boyutlu dizilerın kullanımını sınamak için sık sorulan bir sorudur. </br>
