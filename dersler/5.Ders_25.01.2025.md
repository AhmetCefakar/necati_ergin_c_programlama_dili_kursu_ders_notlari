...
❗❗❗ Onlalıklı sayı(floating point numbers) türlerinde işaretli/işaretsiz ayrımı yoktur, mesela `signed float x;` şeklinde bir değişken bildirimi yapılamaz.

❗❗❗ Tam sayı türlerinin temel türü "_int_" türüdür.

❗❗❗ Küsuratlı sayıların(floating point numbers) temel türü "_double_" türüdür.
...
❗❗❗ **Indetermined Value**: "**automatic**" ömre sahip değişkenlere ilk değerleri verilmediğinde bu değişkenler bellekte kendilerine tahsis edilen alandaki değer olduğu şekliyle ömürlerine başladıklarında sahip oldukları ne olduğu belli olmayan değere verilen addır, bu durum için standardın kullanmadığı "**garbage value(çöp değer)**" tanımlaması da kullanılabiliyor.
...
👇 Aşağıdaki programın bir mülakatta yorumlanması istendiğinde verilecek cevap "x" değişkeninin "**indetermined value**" değer alacağı için "**ub**" durumunun oluşacağıdır.
